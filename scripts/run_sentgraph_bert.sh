...
if [ $DATASET == norec ]; then
    ...
    EXTERNAL=vectors/20/18.zip
elif [ $DATASET == vietnam ]; then
    EXTERNAL=vectors/model.txt
    # EXTERNAL=vectors/cc.vi.300.vec
else
    echo "NO EMBEDDINGS SUPPLIED FOR THIS DATASET"
    echo "EXITING TRAINING PROCEDURE"
    exit
fi
echo BERT model: $MODEL
...
TRAIN_CV=data/sent_graphs/$DATASET/$MODEL/train_bert.hdf5
DEV_CV=data/sent_graphs/$DATASET/$MODEL/dev_bert.hdf5
TEST_CV=data/sent_graphs/$DATASET/$MODEL/test_bert.hdf5
...
DIR=experiments/$DATASET/$MODEL/$SETUP/$RUN
